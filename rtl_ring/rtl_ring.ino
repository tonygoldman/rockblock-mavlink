...
void setup()
{
  ...
  pinMode(4, OUTPUT); // Configure the Pixhawk pin
  ...
  Serial.println("Setting rtl pin low");
  digitalWrite(D4, LOW); // pixhawk gpio 
  ...
  Serial.println("Begin wait for Ring...");
...
void loop()
{
  ...
  if ((ring == true) || (modem.getWaitingMessageCount() > 0) || (err != ISBD_SUCCESS))
  {
    if (ring == true) {
      ...
      digitalWrite(D4, is_on ? LOW : HIGH); // pixhawk gpio 
      is_on = !is_on;
    }
